...
/**
 * transaksi.php
 * Manajemen Transaksi Kas Kelas — Pencatatan pemasukan & pengeluaran, upload bukti, filter, dan ekspor laporan PDF.
 */

require_once 'config/database.php';

// 1. Tangani Hapus Transaksi (SEBELUM output HTML agar redirect berjalan mulus)
...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_SESSION['user_id'])) {
    ...
    if ($id > 0) {
        try {
            // Ambil dan hapus file bukti jika ada
            ...
            $_SESSION['flash_success'] = 'Data transaksi berhasil dihapus dari database!';
...
// Ambil Saldo Kas Aktual Global untuk validasi
$ringkasan = get_ringkasan_kas($pdo);
$actual_saldo_kas = $ringkasan['saldo'];

// 2. Tangani Penambahan Transaksi (Hanya Admin/Bendahara)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_type']) && $_POST['action_type'] === 'add') {
    ...
    // Handle upload file bukti nota (opsional)
    $bukti_path = null;
    $has_upload = isset($_FILES['bukti']) && $_FILES['bukti']['error'] !== UPLOAD_ERR_NO_FILE;

    if ($jenis === 'pengeluaran' && $has_upload) {
        ...
        if ($upload_err !== UPLOAD_ERR_OK) {
            ...
            $error = 'Gagal upload bukti: ' . ($err_msgs[$upload_err] ?? 'Kode error ' . $upload_err);
        } else {
            ...
            if ($_FILES['bukti']['size'] > $max_size) {
                ...
                if (!in_array($ext, $allowed_ext)) {
                    $error = 'Format file bukti harus berupa gambar (JPG, PNG, WebP)!';
                } else {
                    $detected_mime = @mime_content_type($_FILES['bukti']['tmp_name']);
                    if (!$detected_mime || strpos($detected_mime, 'image/') !== 0) {
                        $error = 'File bukti tidak dikenali sebagai gambar yang valid.';
                    } else {
                        $filename = 'bukti_' . date('Ymd_His') . '_' . uniqid() . '.' . $ext;
                        $dest = __DIR__ . '/assets/uploads/' . $filename;
                        if (move_uploaded_file($_FILES['bukti']['tmp_name'], $dest)) {
                            $bukti_path = 'assets/uploads/' . $filename;
                        } else {
                            $error = 'Gagal menyimpan file bukti. Periksa hak izin (permission) folder uploads.';
                        }
                    }
                }
            }
    ...
    if ($error === '') {
        ...
        if ($jumlah <= 0 || $keterangan === '') {
            ...
            try {
                ...
                // Validasi saldo aktual dengan kunci FOR UPDATE
                $stmt_lock = $pdo->query("
                    SELECT 
                    ...
                    AS saldo 
                    FROM transaksi FOR UPDATE
                ");
                $current_lock_balance = (float)($stmt_lock->fetch()['saldo'] ?? 0);

                if ($jenis === 'pengeluaran' && $jumlah > $current_lock_balance) {
                    throw new Exception('Saldo kas tidak mencukupi untuk pengeluaran ini! Saldo saat ini: ' . format_rupiah($current_lock_balance));
                }
                ...
                $pdo->commit();
                $success = 'Transaksi kas berhasil disimpan ke database!';

                // Refresh saldo kas aktual
...
if ($filter_jenis !== 'semua' && in_array($filter_jenis, ['pemasukan', 'pengeluaran'])) {
    $conditions[] = "t.jenis = ?";
    $params[] = $filter_jenis;
}
if ($filter_bulan !== '' && $filter_bulan >= 1 && $filter_bulan <= 12) {
...
try {
    $query = "
        SELECT t.*, a.nama AS nama_anggota, a.nis AS nis_anggota, u.nama AS nama_petugas 
        FROM transaksi t
...
<div class="space-y-6">
        ...
        <form method="GET" action="transaksi.php" class="grid grid-cols-2 sm:grid-cols-4 gap-3 flex-grow max-w-4xl">
            ...
            <div>
                ...
                <select name="filter_tahun" id="filter_tahun" class="input select text-xs">
                    ...
                    ?>
                        <option value="<?= $y ?>" <?= $y === (int)$filter_tahun ? 'selected' : '' ?>><?= $y ?></option>
                    <?php endfor; ?>
                </select>
            </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
        <div class="card p-3 md:p-4 flex items-center justify-between card-hover">
            <div class="min-w-0">
                ...
                <span style="font-size:14px;font-weight:bold;color:var(--income);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:block">
                    <?= format_rupiah($total_pemasukan_filter) ?>
                </span>
            </div>
        ...
        <div class="card p-3 md:p-4 flex items-center justify-between card-hover">
            <div class="min-w-0">
                ...
                <span style="font-size:14px;font-weight:bold;color:var(--expense);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:block">
                    <?= format_rupiah($total_pengeluaran_filter) ?>
                </span>
            </div>
        ...
        <div class="card p-3 md:p-4 flex items-center justify-between card-hover">
            <div class="min-w-0">
                ...
                <span style="font-size:14px;font-weight:bold;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:block;color:<?= $saldo_filter >= 0 ? 'var(--primary-600)' : 'var(--expense)' ?>">
                    <?= format_rupiah($saldo_filter) ?>
                </span>
            </div>
            <div style="width:32px;height:32px;border-radius:8px;background:var(--tab-active-bg);color:var(--primary-600);display:flex;align-items:center;justify-content:center;font-size:12px;flex-shrink:0;margin-left:8px">
    ...
    <div class="card overflow-hidden">
        <div class="card-header">
            ...
            <p style="font-size:12px;color:var(--text-muted);margin-top:2px">Ditemukan <strong><?= count($transactions) ?></strong> data transaksi di database</p>
        </div>
            ...
            <table class="data-table" id="transTable">
                <thead>
                    <tr>
                        ...
                        <th>Keterangan</th>
                        <th style="width:180px">Pembayar / Anggota</th>
                            ...
                            <tr>
                                ...
                                <td class="font-semibold">
                                    ...
                                    <?php if ($trans['minggu'] && $trans['bulan'] && $trans['tahun']): ?>
                                        <span class="block text-[10px] text-indigo-500 font-medium mt-0.5">
                                            <i class="fa-regular fa-calendar-check mr-0.5"></i> Kas Minggu <?= (int)$trans['minggu'] ?>, <?= $nama_bulan[(int)$trans['bulan']] ?? '' ?> <?= (int)$trans['tahun'] ?>
                                        </span>
                                    <?php endif; ?>
                                </td>
                                <td class="font-medium" style="color:var(--text-muted)">
                                    <?php if ($trans['nama_anggota']): ?>
                                        <?= e($trans['nama_anggota']) ?>
                                        <?php if ($trans['nis_anggota']): ?>
                                            <span style="display:block;font-size:9px;color:var(--text-dim);font-family:monospace">NIS: <?= e($trans['nis_anggota']) ?></span>
                                        <?php endif; ?>
                                    <?php elseif ($trans['jenis'] === 'pemasukan'): ?>
                                        <span style="color:var(--text-dim);font-size:11px">Kas Umum Kelas</span>
                                    <?php else: ?>
                                ...
                                <td class="text-center">
                                    <?php if (!empty($trans['bukti'])): ?>
                                        <a href="javascript:void(0)" onclick="previewBukti('<?= e($trans['bukti']) ?>')"
                                ...
                                <td class="text-center">
                                    <a href="transaksi.php?action=delete&id=<?= (int)$trans['id'] ?>"
                                       onclick="return confirm('Apakah Anda yakin ingin menghapus data transaksi ini? Penghapusan akan memicu perubahan pada saldo kas.')"
        ...
        <form action="transaksi.php" method="POST" id="transactionForm" class="p-5 md:p-6" enctype="multipart/form-data" style="display:flex;flex-direction:column;gap:18px">
                    ...
                    <div>
                        ...
                        <select name="anggota_id" id="anggota_id" class="input text-xs font-semibold select">
                            ...
                            <?php foreach ($anggota_list as $member): ?>
                                <option value="<?= (int)$member['id'] ?>"><?= e($member['nama']) ?> (<?= e($member['nis'] ?? '-') ?>)</option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="grid grid-cols-3 gap-2">
                        <div>
                            <label for="minggu" class="input-label">Mgg</label>
                            <select name="minggu" id="minggu" class="input text-xs select">
                                <?php $c_w = (int)ceil(date('j')/7); for ($w = 1; $w <= 5; $w++): ?>
                                    <option value="<?= $w ?>" <?= $c_w === $w ? 'selected' : '' ?>>Mg <?= $w ?></option>
                                <?php endfor; ?>
                            </select>
                        </div>
                        <div>
                            <label for="bulan" class="input-label">Bln</label>
                        ...
                        <div>
                            <label for="tahun" class="input-label">Thn</label>
                            <select name="tahun" id="tahun" class="input text-xs select">
                                <?php $c_y = (int)date('Y'); for ($y = $c_y - 2; $y <= $c_y + 2; $y++): ?>
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" required rows="2"
                          ...
                          placeholder="Misal: Uang kas mingguan, beli spidol & penghapus, isi ulang air galon, dll"></textarea>
            </div>
            ...
            <div id="pengeluaranFields" style="display:none">
                <div style="padding:14px;border-radius:10px;border:1px solid var(--border);background:var(--surface-bg);display:grid;gap:12px">
                    <div>
                        <label for="pengguna_id" class="input-label"><i class="fa-solid fa-user mr-1"></i> Yang Mengeluarkan (Opsional)</label>
                    ...
                    <div>
                        ...
                        <p style="font-size:10px;color:var(--text-dim);margin-top:4px;display:flex;align-items:center;gap:4px"><i class="fa-solid fa-circle-info"></i> JPG, PNG, atau WebP, maksimal 2MB</p>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-header">
            <div style="display:flex;align-items:center;gap:10px">
                ...
                <div>
                    ...
                    <p style="margin:1px 0 0;font-size:10px;color:var(--text-muted)">Dokumen fisik pendukung transaksi kas kelas</p>
                </div>
            </div>
...
const currentActualBalance = <?= (float)$actual_saldo_kas ?>;
...
function updateDuesKeterangan() {
    ...
    if (selectAnggota && selectAnggota.selectedIndex > 0 && selectMinggu && selectBulan && selectTahun) {
        const namaSiswa = selectAnggota.options[selectAnggota.selectedIndex].text.split(" (NIS")[0].trim();
...
function exportLaporanPDF() {
    ...
    sigImg.onerror = function() { generatePDF(null); };
}
...
function generatePDF(sigImgData) {
    ...
    const C = {
        ...
        accentBlue: [79, 70, 229], black: [15, 23, 42]
    };
    ...
    doc.setDrawColor(...C.black); doc.setLineWidth(0.8); doc.line(ML + 2, logoY + 16, PW - MR - 2, logoY + 16);
    doc.setLineWidth(0.25); doc.line(ML + 2, logoY + 17, PW - MR - 2, logoY + 17);

    const infoY = logoY + 24, cols3 = CW / 3;
    const infoItems = [
    ...
    infoItems.forEach((item, idx) => {
        ...
        doc.setFillColor(250, 251, 253); doc.setDrawColor(...C.borderGray); doc.setLineWidth(0.3);
        doc.roundedRect(ix, infoY - 4, cols3 - 4, 14, 1.5, 1.5, 'FD');
        doc.setFont('helvetica', 'bold'); doc.setFontSize(6.5); doc.setTextColor(...C.subText);
    ...
    for (let i = 0; i < tbodyRows.length; i++) {
        ...
        rows.push({
            no, tanggal, jenis, keterangan, anggota, jumlahNum, isPemasukan,
    ...
    doc.autoTable({
        ...
        body: rows.map(r => [r.no, r.tanggal, r.jenis.toUpperCase(), r.keterangan, r.anggota || '-', r.jumlahStr]),
        theme: 'grid',
        headStyles: {
            fillColor: C.headerBg, textColor: C.headerText, fontStyle: 'bold', fontSize: 8.5,
            lineColor: C.borderGray, lineWidth: 0.25, cellPadding: { top: 5, bottom: 5, left: 4, right: 4 }, valign: 'middle'
        },
        columnStyles: {
    ...
    const sumW = CW, sumX = ML + 2, rowH = 7;
    let sy = afterTableY;
    ...
    sumItems.forEach((item, idx) => {
        ...
        doc.setFont('helvetica', idx === 2 ? 'bold' : 'normal'); doc.setFontSize(7); doc.setTextColor(...C.subText);
        doc.text(item.label, sumX + 3, sy + 5);
        doc.setFont('helvetica', 'bold'); doc.setFontSize(idx === 2 ? 8 : 7.5); doc.setTextColor(...item.color);
