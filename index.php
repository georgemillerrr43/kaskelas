...
function genSiswaPDF(sigImgData) {
    ...
    doc.text('DAFTAR SISWA — STATUS PEMBAYARAN KAS KELAS', tx, y+4);
    ...
    doc.text('Periode: '+bulan+' '+tahun+' — Sistem Informasi Keuangan Uangkas Kelas', tx, y+9);
    // Tgl cetak kanan
    ...
    doc.autoTable({
        ...
        body:rows,
        theme:'grid',
        headStyles:{fillColor:C.headerBg,textColor:C.headerText,fontStyle:'bold',fontSize:7.5,lineColor:C.borderGray,lineWidth:0.25,cellPadding:{top:3.5,bottom:3.5,left:2,right:2}},
        columnStyles:{
        ...
        styles:{font:'helvetica',fontSize:7.5,cellPadding:{top:3,bottom:3,left:2,right:2},lineColor:C.borderGray,lineWidth:0.2,valign:'middle',textColor:C.black},
...
function genRiwayatPDF(sigImgData) {
    ...
    const C = {
        white:[255,255,255], pageGray:[245,247,250], borderGray:[200,210,220],
        headerBg:[240,243,248], headerText:[20,25,35], subText:[70,80,95],
        dimText:[145,155,170], rowAlt:[247,249,252], incomeText:[4,120,87],
        incomeBg:[235,252,240], expText:[190,18,60], expBg:[255,240,242],
        black:[15,20,30], accentBlue:[75,70,225],
    };
    // Page bg
    doc.setFillColor(...C.pageGray); doc.rect(0,0,PW,PH,'F');
    doc.setFillColor(...C.white); doc.roundedRect(10,10,PW-20,PH-20,3,3,'F');
    ...
    doc.setFont('helvetica','normal'); doc.setFontSize(8); doc.setTextColor(...C.subText);
    doc.text('Seluruh aktivitas keuangan — Sistem Informasi Uangkas Kelas', tx, y+9);
    const now=new Date(), tglCetak=now.toLocaleDateString('id-ID',{day:'2-digit',month:'long',year:'numeric'});
    const jamCetak=now.toLocaleTimeString('id-ID',{hour:'2-digit',minute:'2-digit'})+' WIB';
    doc.setFont('helvetica','bold'); doc.setFontSize(7.5); doc.setTextColor(...C.subText);
    doc.text('Tanggal Cetak', PW-12, y+2, {align:'right'});
